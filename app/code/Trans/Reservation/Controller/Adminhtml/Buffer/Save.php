...
class Save extends \Trans\Reservation\Controller\Adminhtml\Config
{
    ...
    public function execute()
    {
        ...
        if($data) {
            $data[ReservationConfigInterface::CONFIG] = ReservationConfigInterface::CONFIG_BUFFER;
            ...
            $validateValue = $this->validateValue($data[ReservationConfigInterface::VALUE], ReservationConfigInterface::CONFIG_BUFFER);
            ...
            if(!empty($data[ReservationConfigInterface::PRODUCT_SKU])) {
                ...
                $validateProduct = $this->validateProduct($data[ReservationConfigInterface::PRODUCT_SKU], ReservationConfigInterface::CONFIG_BUFFER);
            ...
            if(!empty($data['data']['category_ids'])) {
                ...
                $validateCategory = $this->validateCategory($categoryIds, ReservationConfigInterface::CONFIG_BUFFER);
            ...
            try {
                $this->logger->info("===========Start Save Product Buffer=============");
                ...
                $this->messageManager->addSuccessMessage(__('You saved the product buffer.'));
                $this->logger->info("success");
            } catch (LocalizedException $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
                $this->logger->info("Error save product buffer. Message = " . $e->getMessage());
            } catch (\Exception $e) {
                $this->messageManager->addExceptionMessage($e, __('Something went wrong while saving the product buffer.'));
                $this->logger->info("Error save product buffer. Message = " . $e->getMessage());
            }

            $this->logger->info("===========End Save Product Buffer=============");
