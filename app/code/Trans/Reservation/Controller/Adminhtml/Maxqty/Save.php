...
class Save extends \Trans\Reservation\Controller\Adminhtml\Config
{
    ...
    public function execute()
    {
        ...
        if($data) {
            $data[ReservationConfigInterface::CONFIG] = ReservationConfigInterface::CONFIG_MAXQTY;

            if(empty($data[ReservationConfigInterface::TITLE])) {
                $data[ReservationConfigInterface::TITLE] = $this->generateTitle(ReservationConfigInterface::CONFIG_MAXQTY);
            }
            ...
            if ($data[ReservationConfigInterface::FILTER] === ReservationConfigInterface::FILTER_CATEGORY) {
                ...
                if(!isset($data[ReservationConfigInterface::PRODUCT_SKU])) {
                    $this->messageManager->addErrorMessage(__('Product SKU is required.'));
                    $resultRedirect->setUrl($this->_redirect->getRefererUrl());
                    return $resultRedirect;
                }
                unset($data['data']['category_ids']);
                $data[ReservationConfigInterface::CATEGORY_ID] = '';
            } else if ($data[ReservationConfigInterface::FILTER] === ReservationConfigInterface::FILTER_ALL) {
                if(!isset($data[ReservationConfigInterface::PRODUCT_SKU])) {
                    $this->messageManager->addErrorMessage(__('Product SKU is required.'));
                    $resultRedirect->setUrl($this->_redirect->getRefererUrl());
                    return $resultRedirect;
            ...
            $validateValue = $this->validateValue($data[ReservationConfigInterface::VALUE], ReservationConfigInterface::CONFIG_MAXQTY);
            ...
            if(!empty($data[ReservationConfigInterface::PRODUCT_SKU])) {
                ...
                $validateProduct = $this->validateProduct($data[ReservationConfigInterface::PRODUCT_SKU], ReservationConfigInterface::CONFIG_MAXQTY);
            ...
            if(!empty($data['data']['category_ids'])) {
                $validateCategory = $this->validateCategory($data['data']['category_ids'], ReservationConfigInterface::CONFIG_MAXQTY);
            ...
            try {
                $this->logger->info("=========== Start Save Reservation Hours =============");
                ...
                $this->messageManager->addSuccessMessage(__('You saved the reservation hours.'));
                $this->logger->info("success");
            } catch (LocalizedException $e) {
                $this->messageManager->addErrorMessage($e->getMessage());
                $this->logger->info("Error save max qty product config. Message = " . $e->getMessage());
            } catch (\Exception $e) {
                $this->messageManager->addExceptionMessage($e, __('Something went wrong while saving max qty product config.'));
                $this->logger->info("Error save max qty product config. Message = " . $e->getMessage());
            }

            $this->logger->info("=========== End Save Max Qty Product Config =============");
