...
HTML;
    ...
    protected function _afterElementHtml($element)
    {
        $htmlId = $element->getId();
        $return = <<<HTML
            <a href="javascript:void(0)" onclick="MultiProductChooser.displayChooser('{$htmlId}-container')" class="widget-option-chooser" title="Open Chooser">
                <img src="{$this->getViewFileUrl('images/rule_chooser_trigger.gif')}" alt="Open Chooser" />
            </a>
            <a href="javascript:void(0)" onclick="MultiProductChooser.hideChooser('{$htmlId}-container')" title="Apply">
                <img src="{$this->getViewFileUrl('images/rule_component_apply.gif')}" alt="Apply">
            </a>
            ...
            <script>
                    ...
                    var MultiProductChooser = {
                        displayChooser : function(chooser) {
                            ...
                            var entities = chooser.up('div.chooser_container').down('input[type="text"].entities').value;
                            var postParameters = {selected: entities};
                            url = '{$chooserUrl}';
                            if (chooser && url) {
                        ...
                        checkProduct : function(event) {
                            ...
                            var pickedSku = mat.childNodes[0].nodeValue;
